"""
ABSOLUTE FINAL WORKING FIX for CELL 07-07

The issue: avg_adapted_params are detached, so pseudo_loss has no gradients.
Solution: Don't detach during inner loop, accumulate query losses directly.

This is a cleaner FOMAML implementation.
"""

fixed_cell = '''# [CELL 07-07] MAML meta-training loop (FOMAML)
...
print(f"[CELL 07-07] Using First-Order MAML (FOMAML)")
...
for meta_iter in range(num_meta_iterations):
    ...
    for user_id in sampled_users:
        ...
        original_params = [p.clone() for p in meta_model.parameters()]
        ...
        # ===== OUTER LOOP: Compute query loss with adapted params =====
        # Now compute query loss WITH gradients for meta-update
        ...
        with torch.no_grad():
            for param, orig in zip(meta_model.parameters(), original_params):
                param.copy_(orig)
